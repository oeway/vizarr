...
describe('URL Parameter Parsing', () => {
  ...
  describe('Configuration Type Conversion', () => {
    test('should handle string to number conversion for numeric parameters', () => {
      ...
      if (config.opacity) config.opacity = parseFloat(config.opacity);
      if (config.channel_axis) config.channel_axis = parseInt(config.channel_axis);
    ...
    test('should handle JSON-like array parameters', () => {
      const contrastLimits = JSON.stringify([[0, 255], [0, 1000]]);
      ...
      expect(config.contrast_limits).toEqual([[0, 255], [0, 1000]]);
    });
  });
}); 